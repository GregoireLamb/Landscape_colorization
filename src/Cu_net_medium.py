...
class Cu_net_medium(nn.Module):
  def __init__(self, n_class=128):
    super(Cu_net_medium, self).__init__()
    self.name = "Cu_net_medium"
    self.n_class = 105
    ...
    deconv1_1 = nn.ConvTranspose2d(in_channels=256, out_channels=256, kernel_size=3, stride=1, padding=1, bias=False)#64
    deconv1_2 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=4, stride=2, padding=1, bias=False)#128
    ...
    deconv2_2 = nn.ConvTranspose2d(in_channels=self.n_class, out_channels=self.n_class, kernel_size=3, stride=1, padding=1, bias=False)#256
    deconv3_1= nn.ConvTranspose2d(in_channels=self.n_class+64, out_channels=self.n_class, kernel_size=3, stride=1, padding=1, bias=False)#256
    deconv3_2= nn.ConvTranspose2d(in_channels=self.n_class, out_channels=self.n_class, kernel_size=3, stride=1, padding=1, bias=False)#256
    ...
    self.deconv1 = nn.Sequential(deconv1_1, nn.BatchNorm2d(256), nn.Sigmoid(),
