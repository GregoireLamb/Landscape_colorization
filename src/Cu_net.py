...
class Cu_net(nn.Module):
    def __init__(self, n_class=128):
        ...
        deconv1_1 = nn.ConvTranspose2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1,
                                       bias=False)  # 32
        deconv1_2 = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=4, stride=2, padding=1,
                                       bias=False)  # 64
        deconv2_1 = nn.ConvTranspose2d(in_channels=256 + 256, out_channels=256, kernel_size=3, stride=1, padding=1,
                                       bias=False)  # 64
        deconv2_2 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=4, stride=2, padding=1,
                                       bias=False)  # 128
        deconv3_1 = nn.ConvTranspose2d(in_channels=128 + 128, out_channels=128, kernel_size=3, stride=1, padding=1,
                                       bias=False)  # 128
        deconv3_2 = nn.ConvTranspose2d(in_channels=128, out_channels=self.n_class, kernel_size=4, stride=2, padding=1,
                                       bias=False)  # 256
        deconv4_1 = nn.ConvTranspose2d(in_channels=self.n_class + 64, out_channels=self.n_class, kernel_size=3,
                                       stride=1, padding=1, bias=False)  # 256
        deconv4_2 = nn.ConvTranspose2d(in_channels=self.n_class, out_channels=self.n_class, kernel_size=3, stride=1,
                                       padding=1, bias=False)  # 256
        ...
        self.conv4 = nn.Sequential(conv4_1, nn.BatchNorm2d(512), nn.Sigmoid(),
                                   conv4_2, nn.BatchNorm2d(512), nn.Sigmoid())

        self.deconv1 = nn.Sequential(deconv1_1, nn.BatchNorm2d(512), nn.Sigmoid(),
                                     deconv1_2, nn.BatchNorm2d(256), nn.Sigmoid())
        self.deconv2 = nn.Sequential(deconv2_1, nn.BatchNorm2d(256), nn.Sigmoid(),
                                     deconv2_2, nn.BatchNorm2d(128), nn.Sigmoid())
        self.deconv3 = nn.Sequential(deconv3_1, nn.BatchNorm2d(128), nn.Sigmoid(),
