...
class Cu_net(nn.Module):
  def __init__(self, n_class=128):
    ...
    deconv2_1 = nn.ConvTranspose2d(in_channels=256+256, out_channels=256, kernel_size=3, stride=1, padding=1, bias=False)#64
    deconv2_2 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=4, stride=2, padding=1, bias=False)#128
    deconv3_1= nn.ConvTranspose2d(in_channels=128+128, out_channels=128, kernel_size=3, stride=1, padding=1, bias=False)#128
    deconv3_2= nn.ConvTranspose2d(in_channels=128, out_channels=self.n_class, kernel_size=4, stride=2, padding=1, bias=False)#256
    deconv4_1= nn.ConvTranspose2d(in_channels=self.n_class+64, out_channels=self.n_class, kernel_size=3, stride=1, padding=1, bias=False)#256
    deconv4_2= nn.ConvTranspose2d(in_channels=self.n_class, out_channels=self.n_class, kernel_size=3, stride=1, padding=1, bias=False)#256
