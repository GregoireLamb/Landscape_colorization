...
def ab2prob(te, n_classes=104):
    ...
    prob[torch.arange(batch_size).unsqueeze(1), ab2class(te, n_classes=n_classes), torch.arange(height).unsqueeze(
        0).unsqueeze(2), torch.arange(width).unsqueeze(0).unsqueeze(1)] = 1
    return prob
...
def ab2class(ab, n_classes=104):
    ...
    """
    if n_classes == 104:
        mapping = (torch.round(ab[:, 0, :, :] * 14) * 15 + torch.round(
            ab[:, 1, :, :] * 14)).clone().long()  # class in a 15*15 grid
...
def compute_PSNR_2_images(image1, image2):
    return 10 * math.log(255 * 255 / (
                compute_euclidean_distance_2_images(image1, image2) * compute_euclidean_distance_2_images(image1,
                                                                                                          image2)), 10)
    ...
    for im_truth in os.listdir(path_folder1):
        for im_pred in os.listdir(path_folder2):
            ...
            if im_truth.split('.')[0] == im_pred.split('.')[0]:
                ...
                if metric == "euclidean":
                    distances.append(compute_euclidean_distance_2_images(transforms.ToTensor()(im_truth),
                                                                         transforms.ToTensor()(im_pred)))
                    break
                elif metric == "PSNR":
                    distances.append(
                        compute_PSNR_2_images(transforms.ToTensor()(im_truth), transforms.ToTensor()(im_pred)))
                    break
                else:
                    print("WARNING: metric not implemented")
            # Add message no image found
    if len(os.listdir(path_folder1)) != len(os.listdir(path_folder2)) or len(os.listdir(path_folder1)) != len(
            distances):
